...
from edgar import fetch_filing_text, parse_hit, search_8k  # noqa: E402
...
def main(startdt=None, enddt=None):
    ...
    seen = load_seen()
    hits = [parse_hit(h) for h in search_8k(startdt, enddt)]
    ...
    queue = []
    for h in candidates:
        ok, snap, reason = liquidity_gate(h["ticker"])
        seen.add(h["adsh"])
        if not ok:
            continue
        stats["gate_pass"] += 1
        try:
            doc = fetch_filing_text(h["cik"], h["adsh"])
